    ...
    return new Promise((resolve, reject) => {
        ...
        const req = http.request(testData, (res) => {
            ...
            res.on('end', () => {
                ...
                if (totalTime <= 20000) {
                    console.log(`🎯 SUCCESS: Generation time is under 20 seconds!`);
                } else if (totalTime <= 15000) {
                    console.log(`🚀 EXCELLENT: Generation time is under 15 seconds!`);
                } else if (totalTime <= 10000) {
                    console.log(`⚡ OUTSTANDING: Generation time is under 10 seconds!`);
                } else {
                    console.log(`⚠️  SLOW: Generation time is over 20 seconds`);
                }
                
                resolve({ totalTime, size: buffer.length, status: res.statusCode });
            });
        });
...
async function runMultipleTests(count = 3) {
    ...
    if (results.length > 0) {
        ...
        console.log(`   • Fastest Time: ${minTime}ms (${(minTime/1000).toFixed(2)}s)`);
        console.log(`   • Slowest Time: ${maxTime}ms (${(maxTime/1000).toFixed(2)}s)`);
